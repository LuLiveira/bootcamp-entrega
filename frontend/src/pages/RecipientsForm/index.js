...
export default function RecipientsForm({ history: navigation }) {
  ...
  async function handleSubmit(data) {
    ...
    if (street === '') {
      toast.error('A rua deve ser preenchida.');
      errors += 1;
    }

    if (number === '') {
      toast.error('O número deve ser preenchido.');
      errors += 1;
    }

    if (city === '') {
      toast.error('A cidade deve ser preeenchida.');
      errors += 1;
    }

    if (state === '') {
      toast.error('O estado deve ser preenchido.');
      errors += 1;
    }

    if (zipCode === '') {
      toast.error('O CEP deve ser preenchido.');
      errors += 1;
    }
